...
app_image:
	docker build --platform linux/x86_64 -f app/Dockerfile -t recipe_app_zcm9834 .
...
rds_app: model/kmeans.pkl
	docker run -it -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e MYSQL_HOST -e MYSQL_PORT -e MYSQL_USER -e MYSQL_PASSWORD -e DATABASE_NAME -p 5000:5000 recipe_app_zcm9834

# complete pipeline for app
